...
void MainWindow::merge()
{
    ...
    {
        qDebug() << "Okay for now";
        mergeImage = new Image();
        mergeImage->Read(filepath);
        if (!mergeImage->lastRead)
        {
            QMessageBox::information(this, tr("Merge load error"), "Could not load the file", QMessageBox::Warning);
        }
        else
        {
            QStringList choose;
            choose << tr("Side by side") << tr("pixel row");
            bool ok;
            QString chose = QInputDialog::getItem(this, tr("Change colour"), tr("Colour: "), choose, 0, false, &ok);
            if (ok)
            {
                if (chose == "Side by side")
                {
                    if (mergeImage->m_height > image->m_height)
                    {
                        QMessageBox::information(this, tr("Merge size error"), "Selected file is too big for this image", QMessageBox::Warning);
                    }
                    else
                    {
                        QString message = filepath +  " has been opened";
                        QMessageBox::information(this, tr("Merge File Name"), message);
                        image->merge(mergeImage, 0);
                    }
                }
                else if (chose == "pixel row")
                {
                    if (mergeImage->m_height > image->m_height || mergeImage->m_width > image->m_width)
                    {
                        QMessageBox::information(this, tr("Merge size error"), "Selected file is too big for this image", QMessageBox::Warning);
                    }
                    else
        ...
        QMessageBox::information(this, tr("Open"), "Nothing to merge", QMessageBox::Information);
    delete mergeImage;
    setImage();
    updateActions();
}
...
void MainWindow::createActions()
{
    ...
    mergeAct = new QAction(tr("Merge images"), this);
    mergeAct->setEnabled(false);
    mergeAct->setShortcut(tr("Ctrl+M"));
    mergeAct->setStatusTip(tr("Merge"));
...
void MainWindow::updateActions()
{
    ...
    mergeAct->setEnabled(!image->isEmpty());
